...
#import "CBiOSSoundManager.h"
#import "CBLibrary.h"
#import <AudioToolbox/AudioToolbox.h>
#import <Foundation/Foundation.h>

CBiOSSoundManager::CBiOSSoundManager()
:m_volume(1)
{
}
...
void CBiOSSoundManager::initial()
{
    initOpenAL();
}

void CBiOSSoundManager::initOpenAL()
{
...
void CBiOSSoundManager::loadSound(const string fileName)
{
    if( m_sounds.find(fileName) == m_sounds.end() )
	{
        ...
        {
            DebugLog("kSoundEngineErrInvalidFileFormat\n");
        }
...
//        result = AudioFileReadPacketData(fileID, NO, &loadSize, packetDesc, packetsLoaded, &numPackets, outdata);
        ...
        //}
        m_sounds[fileName] = sourceID;
        free(outdata);
        outdata = NULL;
    }
}
//void CBiOSSoundManager::releaseSound(const string fileName)
//{
//    
//}
void CBiOSSoundManager::releaseAllSound()
{
    for (std::map<string,ALuint >::iterator it = m_sounds.begin(); it != m_sounds.end(); ++it)
    {
...
void CBiOSSoundManager::playSound(const string fileName)
{
    if( !(m_sounds.find(fileName) == m_sounds.end()) )
	{
        ALuint sourceID = m_sounds[fileName];
        alSourcef(sourceID, AL_GAIN, m_volume);
        alSourcePlay(sourceID);
    }
}
void CBiOSSoundManager::stopSound(const string fileName)
{
    if( !(m_sounds.find(fileName) == m_sounds.end()) )
	{
        ALuint sourceID = m_sounds[fileName];
...
float CBiOSSoundManager::getVolume()
{
    return m_volume;
}
void CBiOSSoundManager::setVolume(float volume)
{
    if(volume > 1)
        volume = 1.0;
    if(volume < 0)
