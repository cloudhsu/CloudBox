...
CBPropertyAction::CBPropertyAction(Property property, double duration)
...
{
	float currentValue = getCurrent(m_target);
	if( (m_transform == PropertyIncrease && currentValue >= m_property.to) ||
	    (m_transform == PropertyReduce && currentValue <= m_property.to) )
	{
		setProperty(m_target,m_property.property,m_property.to);
		finishAction();
	}
}
